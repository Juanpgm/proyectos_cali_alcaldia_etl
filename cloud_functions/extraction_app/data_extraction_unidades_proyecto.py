...
def standardize_data_structure(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in percentage_columns:
        if col in standardized_df.columns:
            ...
            # Check if values are suspiciously small (likely divided by 100)
            # If max value is <= 1.0 and there are non-zero values, multiply by 100
            ...
            if len(non_zero_values) > 0:
                ...
                # If maximum value is <= 1.0, values are likely stored as decimals (0.XX instead of XX)
                if max_val <= 1.0:
                    print(f"⚠️  Detected percentage stored as decimal in '{col}' (max={max_val:.4f})")
                    print(f"   Multiplying by 100 to convert to percentage format")
